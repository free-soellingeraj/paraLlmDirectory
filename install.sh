...
TTS_STREAM_WORKING_ENABLED="1"   # Soft "heartbeat" sound whenever Claude is working/thinking and the voice is silent (so you know your prompt landed); silence only when idle. 0 = off
# TTS_STREAM_WORKING_SOUND=""     # Path to your own working-sound file; empty = synthesized "sticks" scrape ($TTS_DIR/working-sticks.wav; a tonal working-cricket.wav alternate is generated too)
