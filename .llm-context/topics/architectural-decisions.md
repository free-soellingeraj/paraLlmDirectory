...
**Key mechanics**:
- *Settling*: a line is safe to speak once it's unchanged between two polls; this excludes the mutating tail without understanding the TUI. A small dip (≤3 lines) of the common prefix below the anchor is tail churn (the streaming paragraph / a prompt line gaining text) — those lines are un-anchored and spoken when they settle. A large dip means a real rewrite (clear/screen switch) → re-anchor without re-speaking history.
